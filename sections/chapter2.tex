...
% expression marginalized posterior of observables?
This section deals with general predictions for NS observables, based on the
Bayesian analysis performed in Section~\ref{sec:bayes}. Our predictions for
global properties of the NS are presented and confronted with popular models 
as well as GW170817 event in~\ref{subsec:gw17}. The Bayesian determination of 
the CC transition density and pressure is carried out in~\ref{subsec:cc_bayes}.
...
% lot of differnt functionals, which make differnt predictions.
...
the GW170817 event]{Marginalized posterior for the pressure 
  ...
  likelihood probability $p_{AME2016}$ is accounted for and $p=3$ is fixed. The 
  blue dark and light shaded regions show the $1\sigma$ and $2\sigma$ 
  ...
  represented. The recent constraint on $P(2\rho_{sat})$ inferred from 
...
the pressure at twice nuclear saturation density, $P(2\rho_{sat}) =
...
the GW170817 event]{Marginalized posterior for the NS mass $M$ as a function of 
  ...
  probability $p_{AME2016}$ is accounted for and $p=3$ is fixed. The blue 
  shaded regions show the $1\sigma$, $2\sigma$ and $3\sigma$ confidence 
  intervals. The BSk24 (dashdotted orange line) and PKDD (dotted 
  green line) unified EoS calculated within the metamodeling technique with 
  $p=3$ are represented.}\label{fig:mr_bayes}
\end{figure}
