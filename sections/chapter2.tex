...
% general pres: 
%   Pulsars were identified to rotating NS which produce pulsed 
%   emission (dont know where to put this) -> observations
%   
%   elementary constituent of neutron star -> nucleons
%
%   observations provide constraints for nuclear physics and vice versa
...
would require Vela mass to be very low, $M_{Vela} \lesssim 0.9M_\odot$.
...
The posterior distributions of $Q_{sat}$ and $Q_{sym}$ are only very slighty  
...
marginalized posterior probabilitiy for low-density parameter $b$ is close to
...
parameters of orders $N \leq 4$ for the sets pawssing through both LD and HD 
filters without the constraint on pressure, for
...
The recent GW170817 event, which is the first dectection of GW from a 
...
the GW170817 event]{Marginalized posterior for the pressure 
  ...
  (dashdotted green line), PKDD (dotted red line), and TM1 (solid puprle line) 
...
The two EoS based on funtionals issued from relativistic mean-field theory, 
namely PKDD and TM1, do not meet the the final LD+HD predictions. The reason is 
...
Surpringly, for the LD prediction, we find that $R_{1.4}$ is especially 
...
$90\%$ confidence level, but we find that our posterior slighlty favors larger 
...
are required in order to explain the large glitches occuring in the Vela
...
selected popular functionals. This is not surprinsing because we known that the 
...
filter slightly reinforces the correlation of transiton density and pressure 
...
better controled in our calculation, as permitted by the introduction of 
...
\begin{figure}[!t]
  \begin{center}
    \includegraphics[width=0.411\linewidth]{figures/ntpt_priorp3.pdf}
    \includegraphics[width=0.411\linewidth]{figures/ntpt_hdp3.pdf}\\
    \includegraphics[width=0.411\linewidth]{figures/ntpt_ldp3.pdf}
    \includegraphics[width=0.411\linewidth]{figures/ntpt_ldwpresp3.pdf}\\
...
$P_t$ are strongly correlated in all cases. The Pearson correlation coefficent 
...
For a given EoS, the determination of the fraction of crust moment of inertia,
using Eq.~(\ref{eq:crustmoi}), 
and more generally of any crustal observable, is possible providing the 
...
related to the theoretical description of the glitch phenemon, which consists
...
\begin{figure}[!t]
  ...
  crust moment of inertia of a $1.4M_\odot$]{Marginalized probabilities for the 
    ...
    models. The minimum values needed to justify Vela glitches with (Delsate
    \textit{et al.}~\cite{Delsate2016} and Andersson 
...
\begin{figure}[!t]
  ...
  \end{center}
  \caption[Blabla]{Blabla.}\label{fig:ifrac_vs_m}
\end{figure}

% vela mass must be lower than 1.4 to meet the prediction at the 90\% level,
%   which is also seen in fig. 2.18

\begin{figure}[!t]
  \begin{center}
    \includegraphics[width=0.9\linewidth]{figures/mr_from_g_vela.pdf}
  \end{center}
  \caption[Blabla]{Blabla.}\label{fig:mr_from_g_vela}
\end{figure}
