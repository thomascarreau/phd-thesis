...
\end{equation}
%
where $X_i$ is uniformely distributed from $X_i^{min}$ to $X_i^{max}$. The use 
of a flat prior means that all possible values of $\bm{X}$ are equally
likely a priori.
...
data~\cite{Margueron2018a}. One can separate the model parameters into two 
groups, depending on how well they are experimentally constrained. It consists 
of the low-order isoscalar parameters $E_{sat}$, $n_{sat}$, $K_{sat}$, the 
symmetry energy at saturation density $E_{sym}$, as well as the effective mass 
$m_{sat}^*$ and isospin splitting $\Delta m_{sat}^*/m$ are rather well 
determined by nuclear experiments, with associated relative uncertainties lower 
than $15\%$, and constitute the first group. The second group consists of the
parameters that are still poorly determined, and those that are not accessible 
by nuclear experiments to the present day. While the uncertainties on the 
isoscalar skewness $Q_{sat}$, the slope of the symmetry energy $L_{sym}$, and 
the isovector incompressibility $K_{sym}$ are yet very large, one can expect a 
better accuracy to be reached in the near future. In particular, the
determination of $L_{sym}$ is an highly topical issue nowadays~\cite{Li2014}.
Finally The isovector sqewness $Q_{sym}$ as well as the parameters of order 
$N=4$, $Z_{sat}$ and $Z_{sym}$, are unaccessible in the laboratory up to the
present. In consequence, we explore very large ranges for these parameters.

\subsubsection{Sensitivity analysis of the crust-core transition point} % 3

% cf CC paper

\subsection{Filters} % 0.25

\subsubsection{Constraints on nuclear physics observables} % 2.5
...
\subsubsection{Constraints arising from astrophysical observations and 
physical requirements} % 1.5

% speak ld hd high order parameters
