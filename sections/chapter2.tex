...
% pressure is the derivative of the energy, calculated here for the seven
%   hamiltonians only and therefore few density dependence are explored. This
%   explain why even realistic models such as BSk24 does not fit in the band
%   for the pressure of PNM. It is therefore questionable whether we should
%   consider the pressure bands as a filter. In that sense, we will test the
%   two situations: when both pressure and energy per nucleon are constrained,
%   and when only the energy per nucleon is constrained.
...
% justify pass band filter.
% we enlarge the band to $5\%$ in order to account for other theoretical
%   predictions in an effective way
% why dont we constrain the low density region < 0.10: because the band is thin
...
The filter $w_{HD}$, hereafter referred to as high density (HD) filter, imposes 
general physical constraints to the global density behavior of the functional, 
as follows:
%
\begin{itemize}
...
\end{itemize}
%
where $M_{max}(\bm{X})$ is the maximum mass obtained for the parameter set
$\bm{X}$ and $M_{max}^{obs}$ is the maximum observed NS mass.

% pass band filter for the mass, justify
% explain that we dont consider cromartie max mass (was not published at the
%   time of this study)
% speak ld hd high order parameters

% symmetry energy positive is a debatable hypothesis (see remark of referee)

% explain that we dont consider lvc constraints since once wants to compare to
%   their predictions in the next section, also they were not very defined at
%   the time of this work
% however, one can expect more constraints in the future with nicer and lvc
