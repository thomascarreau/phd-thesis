...
ambitious NICER program \edit{is given more and more constraining information 
  on the NS radii
...
\edit{that appears to be even larger for high-mass NS.}
...
\edit{The studies on crust properties requires a unified modeling, such as
...
%
\edit{This is justified by the fact that the pairing gap in PNM vanishes at
...
mass\footnote{\edit{It differs from the 
    definition Eq.~(\ref{eq:effm}), which corresponds to the $k$-mass $m_{n,k}$ 
    arising from the spatial nonlocality of the mean field. Here, the 
    $\omega$-dependence is introduced because of the transport of neutrons, 
which is a dynamical phenomemon, and the neutron effective mass reads
$m_n^* = m_{n,k} m_{n,\omega}/m_n$, where $m_{n,\omega}$ is the 
$\omega$-mass.}}.
...
once again the notable dependence on the EoS. \edit{In particular, it has been
...
\edit{This is motivated by the fact that the behavior of Skyrme-type 
...
discussed thoroughly in~\cite{Antic2019}. \edit{In this paper, we have 
...
\edit{This can be understood from the fact that the symmetry energy at
...
\edit{Some of the results presented in the following have been published
...
low-density EoS.
%
\edit{The excellent determination of the EoS at $2\rho_{sat}$ comes from the 
   that we have applied the LD constraint up to $0.20$ fm$^{-3}$. If we only
trust the ab initio calculations up to $\rho_{sat}$, because of the 
...
\edit{The only hypothesis of the metamodel is that the EoS can be
written as a Taylor series, which exclude the eventuality of a transition to 
quark-gluon plasma.
...
%
\edit{To obtain the prior distribution, we calculate the surface plus 
...
HD, LD with and without the pressure filter, \edit{and LD+HD with and without 
the pressure filter.} In all cases, $p=3$ is fixed. \edit{The right two columns 
...
\edit{as well as for LD+HD with the pressure filter} are reported in 
Table~\ref{table:ntpt}. 
\edit{For each filter, our predictions for $n_t$ and $P_t$ are compatible with 
...
\begin{figure}[!t]
  ...
  \end{center}
  \caption[\edit{Correlation of the crust thickness and fractional crust moment 
    of inertia of a $1.4M_\odot$ neutron star with the equation of state 
    parameters for different filters}]{\edit{Correlation of the crust thickness 
...
with the isovector sqewness $Q_{sym}$. A small correlation is also obtained
between the fractional crust moment of inertia and the isovector
incompressibility $K_{sym}$. Therefore, one of 
the conclusions drawn in~\cite{Carreau2019moi}, namely the fact that higher 
precision in the experimental determination of $K_{sym}$ and $Q_{sym}$ in the 
low density EFT theoretical predictions is required to reduce the uncertainties 
on crustal observables, hold despite the different fitting protocol employed in 
the present study. However, as observed in Fig.~\ref{fig:corr_ntpt_pars} for 
the CC transition density and pressure, we can see that the correlations with 
the surface and curvature parameters disappear when a refined control of the 
surface energy is considered.
}
...
\end{tabular}
\end{center}
\caption[\edit{$68\%$ confidence intervals for the crust thickness and
fractional crust moment of inertia of $1.4M_\odot$ neutron star for different 
filters}]{\edit{$68\%$ confidence intervals for the crust thickness and 
  ...
  in~\cite{Carreau2019moi}.}}\label{table:ntpt}
\end{table}
%
\edit{
% comme pour nt et pt, commenter et expliquer diff avec PRC
Blabla.
}
