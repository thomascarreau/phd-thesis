...
{observational} data can be used to better constrain the nuclear EoS. 
...
\end{eqnarray}
%
where $G$ is the gravitational constant, $P$ the pressure, {$\varepsilon = 
...
$\Phi(r)$ corresponds to the gravitational potential {in the Newtonian 
...
(\ref{eq:massbal}), which gives the profiles $P(r)$, {$\rho(r)$}, and 
...
EoS, one first has to choose an arbitrary value for the {central mass 
density $\rho_{c}$} and 
...
the mass density {$\rho$} in the table. In the end, for a given value of 
{central mass density $\rho_{c}$}, one obtains the profiles $P(r)$, 
{$\rho(r)$}, and $m(r)$, and therefore a value for the NS mass $M$ and 
...
{The bands correspond to the measured masses of two massive 
...
maximum mass for this EoS, $2.05M_\odot$, is {marginally compatible 
...
2.05M_\odot$. {One can show that the branch left to the point is 
...
transition pressure $P_t$ is essential for the determination of 
the {proper depth} and mass of the crust, given respectively 
by $l_{crust}=R-R_{core}$ and $M_{crust}=M-M_{core}$, 
with $R_{core}$ ($M_{core}$) the radius (mass) of the stellar core. 
Indeed, in order to calculate the core radius and
...
functional, when we integrate the TOV equation, {we redefine the 
...
$R^3\Omega^2/(GM) \ll 1$, $\Omega$ being the angular frequency. {For
...
%
In that case, we have $I_c = I - I_{res} \simeq I$ ({$I_{res}$ being the 
...
mass\footnote{It is important to stress that this effective mass differs 
    ...
    leading to the definition of the $\omega$-mass, {$\omega$ being the 
    frequency}. In this context, the effective mass reads 
    $m^* = m_{k} m_{\omega}/m$, where {$m_{\omega} = 
...
{Theoretically, for the gravitational signal observed before merger, the 
...
with LIGO and Virgo, and radius measurements {from observatories such as 
...
the behavior of the surface tension for extreme isospin values {for a 
given set of empirical parameters}. Either it is fixed to the educated value 
$p=3$, or {the range $p=\{2.5,3,3.5\}$ is explored}. The latter option is 
...
all the EoS model parameters, {that is} the empirical parameters, 
...
BSk24 and those on right (orange) to BSk22. {The parameters are 
  varied one by one around the two different reference sets of parameters.} 
...
ETF bands. Interestingly, the {inferred} posterior band of the 
...
\begin{itemize}
  ...
  \item the {thermodynamic} stability of the EoS, {$dP/d\rho > 0$},
...
mass constraint, {and} relaxing the condition of the positiveness of the 
symmetry energy {does not alter} our results.
...
low-density EoS.
%
{The {very tight} determination of the EoS at $2\rho_{sat}$ comes from the 
...
%
{This feature, and the fact that, for each set of empirical parameters, we
...
$90\%$ confidence level, but we find that our posterior favors slightly larger 
values of $\Lambda_{1,2}$, that can be obtained with stiffer EoS, associated 
to less compact NS. 
%
{This tension arises due to the inclusion of nuclear-physics constraints 
  in our calculation. The only hypothesis of the metamodel is that the EoS can 
  be written as a Taylor series, which excludes the eventuality of a 
  first-order phase transition, notably to quark-gluon plasma.
  ...
  quarks in the core of NS. Indeed, a possible explanation could be that one of 
  the stars has undergone a phase transition and the other not, leading to two 
  different EoS for the two stars. 
  We do not observe such evidence in our results, thus the hypothesis of a 
  purely nucleonic EoS cannot be excluded. Increasing the number of binary NS 
  GW signals will help to conclude on the strength of the tension.}
%
...
{Indeed, in the present study we consider a curvature term, and we 
...
{providing the pinning and effective mass density profiles as well as the 
...
{as the consequence of} the transfer of angular momentum from the neutron 
