...
In our case, we first generate a large number of NM models, 
...
models through constraints, which come from physical requirements, 
astrophysical observations, and ab initio calculations of SNM and PNM. This 
...
We now turn to the description of the parameter set $\bm{X}$. Our calcutation
...
in~\ref{subsubsec:cld}. The parameter $p$ thus completes the parameter set,
which we note in a compact form as $\bm{X} \equiv (\bm{B},p)$.

\subsection{Prior distribution from nuclear experiments}\label{subsec:prior}

% discuss the value of p = 2.5, 3, 3.5

% give description of each parameter (current knowledge blabla)
