...
nuclear masses, which are available up to $(N-Z)/A \lesssim 0.3$, \edit{$N$
  being the number of neutrons, $Z$ the number of protons, and $A=N+Z$ the 
  number of nucleons constituting atomic nuclei.} The 
...
\end{equation}
%
with $E_i$ the ion energy, $V_{WS}$ the volume of the cell, and $\varepsilon_e$ 
the energy density of the \edit{free electron Fermi gas}. 
...
is the mass excess and $m_u$ is the atomic mass unit), $m_e$ the electron mass, 
and $B_e$ the binding energy of atomic electrons depending solely on the 
...
In the cold outer crust, the mass densities are above $\sim 10^4$ g/cm$^3$ therefore the electrons are essentially free. 
\edit{In this regime, it was shown in~\cite{Watanabe2003elec} that 
  electron-charge screening effects are negligible and that the electron 
density is essentially homogeneous. This is explained from the fact that the 
electron Thomas-Fermi screening length is larger than the lattice spacing.} 
...
\begin{eqnarray}
  ...
  \varepsilon_e}{\partial n_e} - \varepsilon_e \notag\\
      &=& \frac{P_r}{8\pi^2}\left[x_r\left(\frac{2}{3}x_r^2 - 1\right)\gamma_r 
        + \ln(x_r + \gamma_r)\right].\label{eq:pel}
\end{eqnarray}
%

\edit{The electron exchange energy, which is a direct consequence of the Pauli
exclusion principle, and the electron correlation energy, which is due to the
fact that the motion of each electron is affected by the motion and position of 
the other electrons, are neglected since they are known to be small in 
comparison with the kinetic energy of relativistic electrons.} 
The expression of the electron exchange correction to the free energy density 
for a strongly degenerate electron gas is given in~\cite{Haensel2007} (see 
their Eq.~(2.151)).
...
\edit{The thermodynamic potential to be minimized is} the zero-temperature 
Gibbs free energy per nucleon at fixed pressure $P$ under the condition of 
charge neutrality $n_e = Zn_N$, until the neutron drip sets in, the condition 
for which is $\mu_n = m_n c^2$, with $\mu_n$ the neutron chemical potential, 
...
the chemical equilibrium is established, that is if weak processes are at 
...
\end{equation}
%
For this reason, it is more convenient to choose the pressure as the 
...
energies from the outdated phenomenological macroscopic model of Myers and 
Swiatecki~\cite{Myers1965} \edit{and considered only 150 even-even nuclei}. 
Moreover, they did not take into account the zero-point vibration energy, 
Eq.~(\ref{eq:ezp}). In the last decades, 
...
to construct microscopic mass tables~\cite{Samyn2002,Goriely2013}. It is 
therefore important to reevaluate the ground state of the outer crust 
considering those experimental and theoretical advances. 
In this line, Haensel and Pichon studied 
...
beginning at $P = 3\times 10^{-11}$ MeV/fm$^{3}$, corresponding approximately 
to $n_B \approx 10^{-9}$ fm$^{-3}$, in order to ensure complete ionization and 
...
\begin{equation}
  S(n_B) = e_{PNM}(n_B) - e_{SNM}(n_B).
\end{equation}

%
At the saturation density
of nuclear matter $n_B = n_{sat} \approx 0.16$ fm$^{-3}$, $S(n_B)$ 
coincides with the so-called symmetry energy parameter $E_{sym}$. This quantity 
is constrained to 32, 31, 30, and 29 MeV for BSk22, BSk23, BSk24, and BSk25, 
respectively. For BSk26, $E_{sym} = 30$ MeV as well but the functional is 
fitted to the APR EoS of PNM~\cite{Akmal1998}, unlike BSk22-25 that are fitted 
to the stiffer LS2 EoS~\cite{Li2008}.}
...
of~\cite{Welker2017} are \edit{not included}. This highlights the importance
...
\begin{equation}
  \mu_e = \frac{\partial \varepsilon_e}{\partial n_e} =
  \frac{3}{4}\hbar c(3\pi^2)^{1/3}n_e^{1/3}.
\end{equation}
%
...
\edit{Let us note that most of the mass of the outer crust is actually 
concentrated in those densest layers.} 
The neutron drip density $n_{ND}$ and pressure $P_{ND}$ slightly depend on 
...
infinite nuclear matter EoS used to calculate the energy of the neutron 
gas, and extended to finite nuclei within the so-called compressible liquid 
drop (CLD) approximation so as to describe the cluster energetics. The 
derivation of the system of equilibrium equations for the determination of the 
inner-crust ground state is detailed in~\ref{subsec:formalism}. The numerical
...
% composition ws cell:
Once the neutron dripline is reached, neutrons start to drip out of nuclei but
...
\caption[{r-cluster and e-cluster representations}]{{Wood-Saxon 
    density profiles, for arbitrary values, within a WS cell in \edit{the 
    regime of the inner crust}. Blue dashed lines correspond to the cluster 
    density with different decompositions between cluster and gas (left,
...
interface. One should ask whether the neutron gas penetrates the cluster or not.
...
distribution
of the $N_{WS}$ neutrons and $Z_{WS}$ protons in the unit cell. In the former, 
the cluster occupies a volume at the center of the cell and is surrounded
...
the gas penetrates the cluster. This interpretation appears spontaneously in
...
represent the neutron gas, characterized by a quasihomogeneous spatial 
...
\edit{One can switch between the two representations using the simple 
geometric relations}
%
\begin{equation}
...
some models, the proton drip could set in the very bottom layers of the
crust~\cite{Pearson2018}. 
However, if nonspherical shapes are considered then it is found that protons 
...
%
\edit{where the cluster asymmetry $I = (N-Z)/A$ has been introduced.}
...
Two limit cases can be distinguished: pure neutron matter, where $n=n_p$,
...
considerable theoretical efforts have been devoted to \textit{ab initio}
...
challenge~\cite{Drischler2016}. \edit{Another possibility is to derive the 
  nuclear EoS from phenomenological forces~\cite{Chabanat1998}. This
approach however} suffers from the possible introduction of artificial 
correlations among the parameter space. In the following we present the 
metamodeling technique~\cite{Margueron2018a,Margueron2018b}. 
As we will see, this technique
allows to parametrize in a simple analytical way the energy per nucleon
obtained in the different \textit{ab initio} or effective approaches, as well as
...
with the empirical parameter $K_{sat}$, which corresponds to the
\edit{compression modulus of SNM}.
%
\begin{table}
\begin{center}
...
\end{equation}
%
where $e_{HM}$ is the energy per nucleon in nuclear matter. In
Table~\ref{table:emp_params} are listed the value of each empirical parameter
for the \edit{Skyrme-type} SLy4~\cite{Chabanat1998}, BSk22, and
...
seen that the isovector parameters are in general less known \edit{than} 
...
$10\%$. It should also be stressed that the high-order parameters $Q_{sat(sym)}$
...
with $x = (n-n_{sat})/(3n_{sat})$ a function of the density. {However, we 
  ...
  The high-order parameters $Q_{sat(sym)}$ and $Z_{sat(sym)}$ are essential and 
...
mass. The Landau effective mass is introduced in order to take into account 
...
{In principle, it is possible to reproduce any EoS model with a Taylor
...
solid lines). A tiny deviation is observed at low density for SNM, but 
...
than $2\%$ at most, and, in the inner crust, the matter inside nuclei is very 
...
\textit{a priori} among the empirical parameters due to the nature of the
...
the compressible liquid drop approximation, originally introduced 
in~\cite{BBP}. This approach is expected to be less accurate than microscopic 
Hartree-Fock (HF)~\cite{Negele1973} or ETF~\cite{Onsi2008} calculations since 
quantum effects are neglected, though it proved its quality in the past by 
giving surprisingly good results, see for 
example \cite{BBP,Lattimer1991,Douchin2000a,Douchin2001}. In particular, it
...
n_0(1-I)/2$ the average proton density inside cluster. Let us note that 
...
the AME2016~\cite{Huang2017} for a fixed value of the parameter $p$. {The
...
made in constraining the NN effective interaction. For this 
...
Using the method of Lagrange multipliers, the ground state of the inner crust 
is obtained by minimizing the zero-temperature free
...
the computational point of view to do so in the \edit{regime of the inner
crust, where nuclei are immersed in a gas of dripped neutrons}.
...
equilibrium condition between the neutrons of the cluster and those of the
gas,
%
\begin{equation}
...
in detail in~\ref{subsec:ccfromc}.
...
force~\cite{Goriely2013}, and the surface parameter $p$ is fixed to the value 
$p=3$, which appears to be a reasonable value for Skyrme-type 
...
Strutinsky integral (TETFSI)~\cite{Onsi2008,Pearson2018}, \edit{as well as 
recent calculations using Gogny D1M$^*$~\cite{Mondal2020}} also predict $Z=40$ 
in the inner crust. As far as CLD calculations are concerned, DH find that the 
equilibrium value of $Z$ is almost constant in the vicinity of $\approx 40$ 
throughout the inner crust, whereas BBP find monotonically increasing values of 
$Z$. Once again, it should be stressed that $Z$ is very sensitive to the isospin
...
heavier clusters. \edit{It is interesting to note that the influence of $p$ on 
  the proton number is similar to that of the slope of the symmetry energy
  $L_{sym}$. 
  In~\cite{Oyamatsu2006}, it was shown that the charge number is larger for the 
EoS having smaller $L_{sym}$ (see their Fig.~3).}
...
increasing density, clusters also become more and more asymmetric, $I$ going
up to $\approx 0.85$ in the bottom layers.

Clusters are found to be more and more dilute with increasing baryon density. 
At the neutron drip density, we get $n_0 \approx 0.145$ fm$^{-3}$, which is 
close to the saturation density of SNM of the BSk24 functional, 
$n_{sat} = 0.1578$ fm$^{-3}$. 
At the CC interface, the cluster density is as low as $n_0
...
slowly increases with increasing depth. Therefore, nuclei become closer and 
closer, eventually becoming close enough to touch each other at some point, 
creating a very large cluster, ultimately leading to homogeneous matter. We 
can also understand this by calculating the volume fraction 
...
%
\begin{equation}
  P = P_g + P_e + P_L,\label{eq:eos_bbp}
\end{equation}
%
at the equilibrium composition,
where $P_L$ is the lattice contribution to the total pressure. \edit{Using
Eq.~(\ref{eq:eos_bbp})} rather than finite differences to estimate the derivative,
...
is that it requires \textit{a priori} a list of magic numbers, which are 
obviously not known in the regime of the inner crust. Let us recall that NV 
found a persistence of $Z=40$ while it is not considered as a magic number in 
ordinary nuclei. 
...
equations using the method of Lagrange multipliers, as 
in \ref{subsec:formalism}, yielding
%
\begin{equation}
...
One of the virtues of the CLD approach is that different geometries for nuclear
...
to $\varepsilon_{surf} = 2\varepsilon_{Coul}$, thus the expression of 
$r$ is analytical,
%
\begin{equation}
  r =
...
phases \edit{(spheres, cylinders, plates, tubes, and bubbles)} by solving 
numerically Eq.~(\ref{eq:rpasta}). Finally, since the bulk 
...
Let us remark that, \edit{as in~\cite{Martin2015}}, we observe pasta phases for 
SLy4, whereas several previous CLD and TF calculations for this functional have 
predicted that the sphere is the most favorable shape at all densities up to 
the CC transition point~\cite{Douchin2000a,Vinas2017}. 
This contradiction can be understood by the fact that we are dealing with very 
tiny differences in energy density (all the shapes are very close), in a region 
where the matter is extremely neutron rich, thus
...
performed~\cite{Carreau2019cc} and is presented in detail in Chapter 2. 
...
NM at beta equilibrium becomes unstable with respect to 
density fluctuations. Two versions of this technique can be distinguished: the 
so-called thermodynamical \cite{Kubis2004,Kubis2007,Gonzalez2017} and 
...
energy functional of homogeneous NM, which is well constrained through
nuclear experiments and/or \textit{ab initio} calculations in the vicinity of 
the nuclear saturation density $n_{sat}$. While it appears as a substantial 
...
\edit{The isovector gradient terms, needed in addition to the EoS}, play a 
...
This observation highlights the importance of keeping the high-order 
parameters $Q_{sat}$ and $Q_{sym}$ ($N=3$) as well as $Z_{sat}$ and $Z_{sym}$ 
($N=4$) in the Taylor expansion. 
...
lower density. 
The effect of varying the low-density parameter $b$ on $n_t$ is also 
...
explicitly included. \edit{For $p \approx 4$, the treatment of the surface 
  tension at extreme isospin is marginal, resulting in numerical instabilities 
...
$3.094\times 10^{-3}$ fm$^{-3}$. This linear equation for $n_t$ is in very
...
Fig.~\ref{fig:cctp_lsym}. Indeed, a full statistical analysis reveals that
\edit{the density and pressure at the transition point to homogeneous matter} 
are correlated with the high-order empirical parameters, which blurs the 
effective correlation with $L_{sym}$.}
...
Following~\ref{subsec:formalism}, the system of \edit{equilibrium equations
determining the composition and EoS of the outer core is
obtained} by minimizing the energy density of matter, Eq.~(\ref{eq:epscore}), 
...
present. \edit{Here, we consider that the core is transparent for neutrinos, 
  which occurs as soon as the temperature falls below $10^9
  - 10^{10}$~K~\cite{Haensel2007}. In this case we can put 
$\mu_{\nu_e} = \mu_{\bar{\nu}_e} = \mu_{\nu_\mu} = \mu_{\bar{\nu}_\mu} = 0$.} 
These two equations, Eq.~(\ref{eq:betaeqcore}) and~(\ref{eq:muemumu}), 
...
which is that of a relativistic FG. A strong model dependence is
...
leading to the so-called ``hyperon puzzle''~\cite{Zdunik2013}. 
\edit{Nevertheless, several existing hyperonic models can fulfill the
constraint~\cite{Oertel2015,Chatterjee2016,Fortin2018}.}
%
In fact, the 
measurement of $\approx 2M_\odot$ NS also rules out \edit{several} models that 
...
interaction. \edit{Let us however mention that several EoS with a quark-hadron
crossover at high density are stiff enough to support NS of masses greater
thatn $2M_\odot$~\cite{Masuda2013,Kojo2015,Baym2019}. In particular, the 
recently proposed QHC19 EoS~\cite{Baym2019}} allows a maximum mass of 
$2.35M_\odot$ and agrees with the constraints on NS properties inferred from 
the GW170817 event \cite{GW1,GW2}.
...
reevaluation of the high-order empirical parameters $Q_{sat}$,
...
Several unified EoS for cold nonaccreting NS have been proposed in the 
...
crust is calculated within the ETFSI approximation~\cite{Onsi2008}. Also, 
  a set of unified EoS built within a relativistic mean-field approach 
  (RMF) was proposed in \cite{Fortin2016} \edit{and another one based on Skyrme 
    functionals in~\cite{Gulminelli2015}.} 
...
the two last methods are expected to be more precise than the CLD 
...
models. {In addition, these more microscopic methods can only be extended 
  ...
  distributions of clusters, that exist at finite temperature, to be 
  considered (see Chapter 3 of this thesis).} 
...
the present day knowledge on experimental masses~\cite{Huang2017,Welker2017} 
supplemented by state-of-the-art microscopic HFB theoretical mass 
tables~\cite{Goriely2013} up to the neutron-drip point. 
The EoS in the inner crust is then calculated
...
equilibrium conditions, that we have solved numerically using Broyden's
method~\cite{Broyden1965}. 
...
technique, namely the fact that no artificial correlations are 
introduced \textit{a priori} among the empirical parameters, to carry out 
complete statistical analyses that will allow us to settle the model dependence 
of the results.}
