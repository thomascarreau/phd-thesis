...
nuclear masses, which are available up to $(N-Z)/A \lesssim 0.3$, {$N$
...
the energy density of the {free electron Fermi gas}. 
...
{In this regime, it was shown in~\cite{Watanabe2003elec} that 
...
{The thermodynamic potential to be minimized is} the zero-temperature 
...
Swiatecki~\cite{Myers1965} {and considered only 150 even-even nuclei}. 
...
of~\cite{Welker2017} are {not included}. This highlights the importance
...
{Let us note that most of the mass of the outer crust is actually 
...
\caption[{r-cluster and e-cluster representations}]{{Wood-Saxon 
    density profiles, for arbitrary values, within a WS cell in {the 
...
{One can switch between the two representations using the simple 
geometric relations}
%
\begin{equation}
...
%
{where the cluster asymmetry $I = (N-Z)/A$ has been introduced.}
...
challenge~\cite{Drischler2016}. {Another possibility is to derive the 
...
for the {Skyrme-type} SLy4~\cite{Chabanat1998}, BSk22, and
...
seen that the isovector parameters are in general less known {than} 
...
the computational point of view to do so in the {regime of the inner
...
Strutinsky integral (TETFSI)~\cite{Onsi2008,Pearson2018}, {as well as 
...
heavier clusters. {It is interesting to note that the influence of $p$ on 
...
where $P_L$ is the lattice contribution to the total pressure. {Using
...
phases {(spheres, cylinders, plates, tubes, and bubbles)} by solving 
...
Let us remark that, {as in~\cite{Martin2015}}, we observe pasta phases for 
...
{The isovector gradient terms, needed in addition to the EoS}, play a 
...
explicitly included. {For $p \approx 4$, the treatment of the surface 
...
{the density and pressure at the transition point to homogeneous matter} 
...
Following~\ref{subsec:formalism}, the system of {equilibrium equations
...
present. {Here, we consider that the core is transparent for neutrinos, 
...
{Nevertheless, several existing hyperonic models can fulfill the
constraint~\cite{Oertel2015,Chatterjee2016,Fortin2018}.}
%
In fact, the 
measurement of $\approx 2M_\odot$ NS also rules out {several} models that 
...
interaction. {Let us however mention that several EoS with a quark-hadron
...
crust is calculated within the ETFSI approximation~\cite{Onsi2008}. Also, 
  ...
  (RMF) was proposed in \cite{Fortin2016} {and another one based on Skyrme 
