...
\subsubsection{Relativistic electron gas}
...
of measuring the mass of odd-nuclei. \edit{The presence of odd-nuclei in the
  outer crust of NS could lead to phase transitions, for instance ferromagnetic, 
  at low temperature. In return, it could generate a magnetic field and alter 
  the existing field, and so the electron gas.}\\
...
Fig.~\ref{fig:clustering}. The cluster and gas density profiles are 
plotted along the WS cell radius in the r-cluster representation (left) and
e-cluster representation (right) using Woods-Saxon profiles with arbitrary
values. Woods-Saxon functions are known to give a good description of 
medium-mass and heavy nuclei density profiles, and are commonly used in
Thomas-Fermi (TF) and extended Thomas-Fermi (ETF) calculations.}\\
...
% description of parameter space of metamodeling:
Nuclear experiments give us knowledge on the properties of nuclear
...
In order to fasten the series convergence, the $\delta^{5/3}$ dependence is 
...
Let us notice in Eq.~(\ref{eq:elfa}) that the energy per particle does not 
...
p\}$. 
% procedure to estimate the surface parameters:
