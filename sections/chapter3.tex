...
\begin{figure}[!t]
  ...
  crust regime]{
    ...
    $T=1\times 10^9$ K (dashdotted orange line), and 
    $T=3\times 10^9$ K (dashed green line).
    Experimental data are supplemented with masses from the microscopic 
...
In~\ref{subsec:strutinsky}, we have proposed a strategy to add Strutinksy shell 
corrections to the CLD energy. Those shell corrections have been tabulated 
in the regime of the inner crust at zero temperature for
four recent functionals of the BSk family, namely BSk22, BSk24, BSk25, and
BSk26~\cite{Pearson2018}.
In the following, we introduce an empirical temperature-dependent factor to the 
zero-temperature shell corrections in the CLD free energy, before studying the 
influence of shell effects at the crystallization temperature.
...
\begin{figure}[!t]
  ...
  crust for BSk14 at finite temperature]{Equilibrium value of proton number $Z$ 
    ...
    functional for different temperatures: $T=0$ MeV (upper panel), $T=0.1$ MeV 
    (middle panel), and $T=1$ MeV (lower panel). Solid lines correspond to the 
...
TETF results -- for which shell corrections are also neglected -- at relatively 
high temperature. 
...
\begin{figure}[!t]
  ...
  thermodynamic conditions in the free neutron regime]{Free energy density as a 
    ...
    MeV (left panels), $T=0.1$ MeV (central panels), and $T=1$ MeV (right 
    panels), and average baryon densities $n_B=3\times 10^{-4}$ fm$^{-3}$ 
    (upper panels), $n_B = 10^{-3}$ fm$^{-3}$ (middle panels), and $n_B = 
    10^{-2}$ fm$^{-3}$ (lower panels). 
...
with surface parameters optimized on ETF calculations are reported.

% Except for BSk25under  the  extreme  and  quite  unrealistic  hypothesis  that  shelleffects  are  not  affected  by  temperature,  all  calculations  withshell  effects  fit  in  the  CLD  band.  This  indicates  that  simpleCLD  modeling  could  be  sufficient  to  study  crust  properties  atcrystallization.
...
\begin{figure}[!t]
  ...
  one-component plasma versus baryon density in the inner crust]{
    Crystallization temperature $T_m$ (upper panel) and 
    equilibrium value of $Z$ (lower panel) as a function of baryon density 
    $n_B$ in the inner crust. The gray band corresponds to extrema for the 
...
\end{figure}
%
% uncertainties on shell corrections are less important than those on the bulk
%   part of the functional, allowing us to neglect the nonsmooth effects in the
%   following, which obviously speed up the MCP calculation

\subsection{Equilibrium composition of the MCP and the importance of the
rearrangement term}\label{subsec:mcp_compo_icrust}

\subsection{Dependence of the impurity parameter on the
EoS}\label{subsec:eos_qimp}
