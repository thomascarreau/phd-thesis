...
microscopic theoretical mass tables~\cite{Goriely2013}. The ion mass $M_{i,e}$ 
in the outer crust is therefore simply calculated using Eq.~(\ref{eq:ionmass}).
...
introducing the effective chemical potential $\tilde{\mu}_q = \mu_q - U_q$. Let 
us note that the chemical potential does not include the rest mass energy in 
the above expressions.

\subsubsection{Energetics of nuclei at finite temperature}

\begin{equation}
  F_{cl}^{(j),CLD} = A^{(j)}f_{HM}(n_0^{(j)},I^{(j)},T) 
  + F_{surf}^{(j)} + F_{curv}^{(j)} + E_{Coul}^{(j)}
\end{equation}
